...
#FIXME
# show more than one drink
            line=$(cat $filename | grep -in "NAME: " | grep -in "$drinkName" \
...
{
    clear
    echo "Up to 7 ingridients"
    for ingNumber in {1..7}
    do
    ...
    clear
    cat $filename | grep -n -i -e "TAGS: \(.*\) ${ingArray[1]}" \
            | grep -i "${ingArray[2]}" | grep -i "${ingArray[3]}" \
            | grep -i "${ingArray[4]}" | grep -i "${ingArray[5]}" \
            | grep -i "${ingArray[6]}" | grep -i "${ingArray[7]}" \
